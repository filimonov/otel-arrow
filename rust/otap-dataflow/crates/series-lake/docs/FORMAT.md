...
The format keeps `format_version=1` and the `v=1` path segment through every
revision below, because no released writer has produced data yet. From the
first release on, the compatibility rules above apply without exception.

### Revision 2 (2026-09-23, pre-release)

Three persisted-format changes, landed together once before the first
release:

- `schema_fingerprint` hashes the crate-owned `series-lake-schema/1`
  rendering of section 3, which includes nullability, instead of Arrow's
...
revision. Attribute-map cells, string-typed denormalized cells and log
bodies holding a non-finite double or a bytes value are spelled differently.

Fingerprints under the default configuration, and for the denormalized
schema of section 3:

```text
schema                      revision 1        revision 2
...
Migration: none for released data, since none exists. A lake written by a
pre-release build mixes two fingerprints for one schema; rewrite it under a
new `base_uri` rather than compacting across the revision.
