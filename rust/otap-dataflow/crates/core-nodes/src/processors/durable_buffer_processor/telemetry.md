...
| `durable_buffer.shutdown.release_thread_failed` | `warn` | No thread could be started to close the storage engine; it is left open, and what it did not persist is replayed from the WAL on the next start. |
