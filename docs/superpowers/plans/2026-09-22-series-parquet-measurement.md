...
- [ ] **Harness minors (7c):** `run_child` must not swallow KeyboardInterrupt; DockerStore removes its container on failed start and applies docker timeouts; `test_disconnect_does_not_remove_data` aligns to a window offset; `free_port()` retries; in `publish=false` mode the 100 ms monitor-coverage gate becomes a recorded observation, not a hard failure.
