...
1. **De-slop round on the campaign branch.** Re-read every comment and piece of prose and cut what is superfluous; re-read every Markdown document and remove what is unneeded or stale; remove from the tree the artifacts that are not needed, temporary documents and other noise (history keeps them). The harness writes raw results to the ignored `.measurement-artifacts/` by default.
   Before the evidence tree under docs/superpowers/reports/ is removed, move the harness's committed inputs out of it: the baseline-*.json files the baseline gate loads (measurement.py EVIDENCE_DIR, load_baselines) and the capacity index the soak reads as its ceiling (soak.py, capacity-minio.json) go to a stable path outside docs/superpowers (or the ceiling is inlined); EVIDENCE_DIR, soak.py and the harness README point there; `measure stage-results --git-add` is removed.
...
- [ ] A nightly-length run (at least 4 h; record the chosen length) per profile that matters most (churn first, then mixed), on one store only: MinIO (user 2026-09-25: one backend is enough for the long run).
